...
class ProcessDashboard(tk.Tk):
    ...
    def _monitoring_process(self, process_name):
        if process_name == "MSSQL":
            exe = "/opt/mssql/bin/sqlservr"
        elif process_name == "MySQL":
            exe = "mysqld"
        elif process_name == "PaySys":
            exe = "Sword3PaySys.exe"
        elif process_name == "RelayServer":
            exe = "S3RelayServer.exe"
        else:
            exe = process_name

        try:
            subprocess.check_output(['pgrep', '-f', exe])
            return True
        except subprocess.CalledProcessError:
            # Nếu là PaySys thì tự động restart bằng jx.sh
            if process_name == "PaySys":
                subprocess.Popen(
                    ["bash", "/root/quanlyserver/2.3.1/jx.sh", "start", "PaySys"],
                    env=os.environ.copy()
                )
            return False
