...
BACKUP_ROOT="$HOME/Desktop/database_backups"
OLD_BACKUP_ROOT="/home/database_backups"
MYSQL_USER="root"
...
function backup_mysql() {
  ...
  local old_backup_file="$OLD_BACKUP_ROOT/${database_name}_${CURRENT_TIME}.sql"
  local new_backup_file="$BACKUP_DIR/${database_name}.sql"

  # Create directories
  mkdir -p "$BACKUP_DIR"
  mkdir -p "$OLD_BACKUP_ROOT"
  chmod 0777 "$OLD_BACKUP_ROOT"
  
  # Dump to OLD location (Original path)
  $MYSQL_DUMP -u $MYSQL_USER -p$MYSQL_PASSWORD -h 127.0.0.1 "$database_name" > "$old_backup_file"

  if [ $? -eq 0 ]; then
    echoFormat "${CYAN}Da luu backup MySQL tai: ${YELLOW}$old_backup_file${NC}"
    # Copy to NEW location (Desktop)
    cp "$old_backup_file" "$new_backup_file"
    echoFormat "${CYAN}Da copy backup sang: ${YELLOW}$new_backup_file${NC}"
  else
...
function backup_mssql() {
  local database_name="$1"
  # Save to OLD_BACKUP_ROOT (which is /home/database_backups)
  local old_backup_path="$OLD_BACKUP_ROOT/${database_name}_${CURRENT_TIME}.bak"
  local new_backup_path="$BACKUP_DIR/${database_name}.bak"
  
  mkdir -p "$OLD_BACKUP_ROOT"
  chmod 0777 "$OLD_BACKUP_ROOT"
  
  # Use a temporary name for the specific timestamped file because MSSQL might overwrite if we use fixed name?
  # No, we use CURRENT_TIME so filename is unique.
  
  # Backup to OLD location
  $MSSQL_CMD -S $MSSQL_SERVER -U $MSSQL_USER -P $MSSQL_PASSWORD -Q "BACKUP DATABASE [$database_name] TO DISK = N'$old_backup_path' WITH FORMAT, INIT"

  if [ $? -eq 0 ]; then
    if [ -f "$old_backup_path" ]; then
        echoFormat "${CYAN}Da luu backup MSSQL tai: ${YELLOW}$old_backup_path${NC}"
        # Copy to NEW location
        cp "$old_backup_path" "$new_backup_path"
        echoFormat "${CYAN}Da copy backup sang: ${YELLOW}$new_backup_path${NC}"
    else
        echoFormat "${RED}Loi: File backup MSSQL khong tim thay tai '$old_backup_path'${NC}"
    fi
  else
...
backup_bxh(){
    echoFormat "Dang chay backupdaemon..."
    cd $GAMEPATH/gateway
    ./backupdaemon
    echoFormat "Da chay xong backupdaemon"
}

backup_silent(){
    CURRENT_TIME=$(date +%Y-%m-%d_%H-%M-%S)
    BACKUP_DIR="$BACKUP_ROOT/$CURRENT_TIME"
    mkdir -p "$BACKUP_DIR"
    chmod 0777 "$BACKUP_DIR"
    
    echoFormat "Bat dau backup db (Silent) vao folder: $BACKUP_DIR"
    
    # Run backupdaemon (BXH)
    backup_bxh
...
restore_backup(){
    ...
    if [ ! -f "$backup_file" ]; then
        echoFormat "${RED}Khong tim thay file backup: $backup_file${NC}"
        return 1
    fi
    ...
    # Remove extension
    db_name="${filename%.*}"
    # Remove timestamp suffix if present (e.g. _2023-10-10_12-30-00)
    db_name=$(echo "$db_name" | sed -E 's/_[0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{2}-[0-9]{2}-[0-9]{2}$//')

    if [ -z "$db_name" ]; then
        echoFormat "${RED}Khong the xac dinh ten database tu file: $filename${NC}"
        return 1
    fi

    if [ "$extension" == "sql" ]; then
        echoFormat "Dang khoi phuc MySQL database: ${CYAN}$db_name${NC} tu ${YELLOW}$filename${NC}"
        $MYSQL_CLIENT -u $MYSQL_USER -p$MYSQL_PASSWORD -h 127.0.0.1 "$db_name" < "$backup_file"
        
        if [ $? -eq 0 ]; then
             echoFormat "${GREEN}Khoi phuc thanh cong MySQL database $db_name${NC}"
        else
             echoFormat "${RED}Loi khi khoi phuc MySQL database $db_name${NC}"
        fi

    elif [ "$extension" == "bak" ]; then
        echoFormat "Dang khoi phuc MSSQL database: ${CYAN}$db_name${NC} tu ${YELLOW}$filename${NC}"
        
        # Staging path for MSSQL (Docker accessible)
        local staging_dir="/home/database_backups"
        mkdir -p "$staging_dir"
        chmod 0777 "$staging_dir"
        local staging_file="$staging_dir/${db_name}_restore.bak"
        
        # Copy file to staging
        echoFormat "Dang copy file vao staging area..."
        cp "$backup_file" "$staging_file"
        
        # MSSQL Restore requires closing connections usually.
        # This command attempts a Force Restore (REPLACE) using the STAGING file path
        $MSSQL_CMD -S $MSSQL_SERVER -U $MSSQL_USER -P $MSSQL_PASSWORD -Q "USE master; ALTER DATABASE [$db_name] SET SINGLE_USER WITH ROLLBACK IMMEDIATE; RESTORE DATABASE [$db_name] FROM DISK = N'$staging_file' WITH FILE = 1, NOUNLOAD, REPLACE; ALTER DATABASE [$db_name] SET MULTI_USER;"
        
        if [ $? -eq 0 ]; then
             echoFormat "${GREEN}Khoi phuc thanh cong MSSQL database $db_name${NC}"
        else
             echoFormat "${RED}Loi khi khoi phuc MSSQL database $db_name${NC}"
        fi
        
        # Cleanup
        rm -f "$staging_file"
    else
        echoFormat "${RED}Dinh dang file khong ho tro: $extension${NC}"
    fi
}
...
elif [ "$arg1" == "backup_silent" ]; then
    backup_silent

elif [ "$arg1" == "backup_bxh" ]; then
    backup_bxh

elif [ "$arg1" == "restore" ]; then
    restore_backup "$arg2"
