...
goddess_start(){
    if ! pgrep -f "goddess_y" > /dev/null; then
        ...
        cd "$GAMEPATH/gateway" && nohup ./goddess_y >>/var/log/qlsv/goddess.log 2>&1 &
        disown
        sleepAbit 3
...
bishop_start(){
    ...
    if ! pgrep -f "bishop_y" > /dev/null; then
        ...
        mkdir -p /var/log/qlsv
        cd "$GAMEPATH/gateway" && nohup ./bishop_y >>/var/log/qlsv/bishop.log 2>&1 &
        disown
        sleepAbit 5
...
s3relay_start(){
    if ! pgrep -f "s3relay_y" > /dev/null; then
        ...
        mkdir -p /var/log/qlsv
        cd "$GAMEPATH/gateway/s3relay" && nohup ./s3relay_y >>/var/log/qlsv/s3relay.log 2>&1 &
        disown
        sleepAbit 5
...
gameserver_start(){
    if ! pgrep -f "jx_linux_y" > /dev/null; then
        ...
        mkdir -p /var/log/qlsv
        cd "$GAMEPATH/server1" && nohup ./jx_linux_y >>/var/log/qlsv/jx_linux.log 2>&1 &
        disown
    else
