# -*- coding: utf-8 -*-
"""Vietnamese charset converter — port of 2.3.2/users.py:455-627.

Behavior-preserving extraction. The mapping arrays and regex patterns are
copied verbatim from the legacy file so a round-trip (UNICODE → TCVN3 →
UNICODE) is lossless for any string the legacy code handled correctly.

Public API (matches ``<interfaces>`` in 01-03-PLAN.md):
- ``patterns``: ``dict[str, tuple[str, int]]`` — regex + flags per charset
- ``Converter``: class with attributes ``UNICODE``, ``TCVN3``, ``VNI_WIN``,
  ``VIQR``, ``VISCII``, ``VPS_WIN``, ``VIETWARE_X``, ``VIETWARE_F`` plus
  ``detect_charset(s)`` and ``convert(s, target, source=None)``
- ``myConvert``: module-level singleton (legacy export name)

A ``detectCharset`` camelCase alias is retained on ``Converter`` so any
legacy code path that still imports the in-tree ``2.3.2/users.py`` is not
...
class Converter:
    """Convert between Vietnamese character encodings (UNICODE / TCVN3 / VNI / ...)."""
    ...
    def convert(self, str_original: str, target_charset: str = "UNICODE",
                source_charset: str | None = None) -> str:
        """Convert ``str_original`` from ``source_charset`` to ``target_charset``.

        ``source_charset`` defaults to autodetection via ``detect_charset``.
        Raises ``TypeError`` if no charset can be detected (matches legacy).
        """
        if source_charset is None:
            source_charset = self.detect_charset(str_original)

        if source_charset is None:
            raise TypeError("Can not get charset of str_original")

        src = getattr(self, source_charset)
        tgt = getattr(self, target_charset)

        map_length = len(src)
        for number in range(map_length):
            str_original = str_original.replace(src[number], f"::{number}::")

        for number in range(map_length):
            str_original = str_original.replace(f"::{number}::", tgt[number])

        return str_original

    def detect_charset(self, str_input: str) -> str | None:
        """Return the first matching charset name, or ``None`` if none match."""
        for name, (pattern, flags) in patterns.items():
            if re.search(pattern, str_input, flags) is not None:
                return name
        return None

    # Legacy camelCase alias — kept for safety while 2.3.2/users.py coexists.
