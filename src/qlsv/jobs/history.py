...
def list_jobs(path: str = JOBS_FILE) -> list[dict[str, Any]]:
...
def append_job(job: dict[str, Any], path: str = JOBS_FILE) -> None:
    """Append ``job`` to the on-disk list, atomically (mode 0600)."""
    jobs = list_jobs(path)
    jobs.append(job)
    write_json(path, jobs, mode=0o600)
...
def prune(
    ...
    for old in dropped:
        ...
        for suffix in (".log", ".exit"):
            try:
                (log_dir / f"{old_id}{suffix}").unlink()
            except OSError:
    ...
    write_json(path, kept, mode=0o600)


def get_job(job_id: str, path: str = JOBS_FILE) -> dict[str, Any] | None:
